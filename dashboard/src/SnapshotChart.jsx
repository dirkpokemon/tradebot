...
export default function SnapshotChart({ trade }) {
  ...
  const candlesRef    = useRef(null);   // full candle array cached after first fetch
  ...
  const computeOverlay = useCallback(() => {
    ...
    // Only show zone overlay in scene 2 (Entry)
  ...
  const renderScene = useCallback((sceneId) => {
    ...
    if (sceneId === 1) {
      ...
      // Entry level (thin, so user sees where the trade will be)
      if (trade.entry_price) {
        addLine(trade.entry_price, "rgba(255,255,255,0.4)", `Entry ${Math.round(trade.entry_price).toLocaleString()}`, 2, 1);
      }

      // Swing points on context candles
      const swings = findSwingPoints(contextCandles, 3);
      // Deduplicate closely spaced swings (keep most extreme within 5 candles)
      const dedupSwings = [];
      ...
      // Show max 6 swing points to avoid noise
      ...
      // Small entry arrow at the right edge of context
    ...
    // ── Scene 2: Entry ──────────────────────────────────────────────────────
      ...
      addLine(trade.tp1, THEME.green,
        `TP1${rrText(trade.tp1)}`, 1, 1);
    ...
    // ── Scene 3: Verloop ────────────────────────────────────────────────────
      ...
      // All price lines with hit indicator
      addLine(trade.entry_price, THEME.white,
      ...
      if (entryTs && entryIdx >= 0) {
        ...
        // Zoom: entry to last event + 10 candles of padding
